...
class VisualGridHuntGame:
    ...
    def get_percept(self) -> dict:
        ...
        wall_ahead = (tuple(front) in self.walls) or not (
            0 <= front[0] < self.width and 0 <= front[1] < self.height)
    ...
    def _front_cell(self):
        dx, dy = [(0, 1), (1, 0), (0, -1), (-1, 0)][self.facing] # Up, Right, Down, Left
...
class GridGameGUI:
    ...
    def draw_grid(self):
        ...
        for tx, ty in self.env.toxic_traps:
            ...
            self.canvas.create_rectangle(x1, y1, x1 + self.cell_size * 0.6, y1 + self.cell_size * 0.6, fill="purple",
                                         outline="black")
...
if __name__ == "__main__":
    ...
    # Usage: python visual_grid_game.py [BFS|DFS|UCS] (default: BFS)
    algo = sys.argv[1] if len(sys.argv) > 1 else 'BFS'

    # Try a larger grid size like 12x12 with 15 food and 3 opponents!
    app = GridGameGUI(root, width=12, height=12, num_food=15, num_opponents=0, algo=algo)
    root.mainloop()